...
reuse_or_translate_node <- function(
  ...
  if (existing_translation) {
    ...
    markdown_lines <- if (first_line(tag) == last_line(tag)) {
      brio::read_lines(source_path)[first_line(tag) + frontmatter_length]
    } else {
      brio::read_lines(source_path)[
        (first_line(tag):last_line(tag) + frontmatter_length)
      ]
    }
...
guess_translate <- function(
  ...
  deepl_part_translate(
    path = fs::path_rel(source, start = repo),
    out_path = fs::path_rel(target, start = repo),
    repo = repo,
    source_lang = source_language,
    target_lang = target_language,
    tail_commit = tail_commit,
